...
the same way.  A dividend future (DIVIDEND_FUTURES) has no index level to scale by, so its
notional is open interest x FUT_VAL_PT x the contract's OWN daily last price, converted the same
way.  Set MEASURE = 'oi' for plain contract counts.
...
PRODUCTS = [
    ...
    ('DED', 'SX5ED'),           # Eurex Euro Stoxx 50 index dividend futures (EUR), annual: DEDZ6, DEDZ7, DEDZ8 ...
    ('ASD', 'SPXDIVAN'),        # CME S&P 500 annual dividend index futures (USD), annual: ASDZ6, ASDZ7, ASDZ8 ...
]
# 1b. Dividend futures, by tab name.  A dividend future has no index level to scale it with, so its
#     notional is open interest x FUT_VAL_PT x the contract's OWN daily last price (PRICE_FIELD,
#     section 7), converted to USD like the others; each contract's price is pulled over the same
#     window as its open interest.  They are annual contracts, so only the DIVIDEND_MONTHS expiries
#     are looked for (() -> every month, like the index futures).  The contract currency is
#     CONTRACT_CURRENCY[tab] (section 7); when the tab is not listed there Bloomberg's CRNCY on the
#     futures is used and a WARNING says so.  Everything else (windows, SKIP_MONTHS, charts) is the same.
DIVIDEND_FUTURES = {'SX5ED', 'SPXDIVAN'}
DIVIDEND_MONTHS = (12,)
...
CONTRACT_CURRENCY = {
    ...
    'SX5ED': 'EUR',     # Eurex Euro Stoxx 50 dividend futures: EUR 100 x dividend points (dividend future: no index to take the ccy from)
    'SPXDIVAN': 'USD',  # CME S&P 500 annual dividend futures: USD 250 x dividend points (dividend future)
}
...
PRICE_FIELD = 'PX_LAST'         # a dividend future's own daily last price (stands in for the index level)
...
EXPECTED_CONTRACT = {
    ...
    'DED': ('EUR', 100), 'ASD': ('USD', 250),                      # Eurex Euro Stoxx 50 dividend / CME S&P 500 annual dividend
}
# ===========================================================================
...
AUDIT_COLUMNS = ['Product', 'Contract', 'Ticker 1-digit', 'Ticker 2-digit', 'Ticker used',
                 ...
                 'Last price']      # the contract's own PRICE_FIELD: filled for dividend futures only
...
class Contract:
    ...
    px_rows: List[Tuple[dt.date, float]] = dataclasses.field(default_factory=list)   # own PRICE_FIELD (dividend futures only)
    note: str = ''

    @property
    def last_price(self):
        return self.px_rows[-1][1] if self.px_rows else None
...
class IndexSeries:
    ...
    For a dividend future (own_price) there is no index: ticker is '', rows stay empty and only
    the FX rate is pulled; each contract's own PRICE_FIELD (Contract.px_rows) takes the index's place.
    """
    product: str                     # tab name
    ticker: str                      # 'KOSPI2 Index' ('' for a dividend future)
    own_price: bool = False          # True: notional uses each contract's own price, not an index
...
# -------------------------------------------------------------- pipeline ---
def is_dividend(name):
    """True for a tab listed in DIVIDEND_FUTURES: notional from the contract's own price."""
    return name in DIVIDEND_FUTURES


def product_months(name, months):
    """The expiry months looked for on one product: every month asked for, except that a dividend
    future keeps only DIVIDEND_MONTHS (annual contracts: December) when that is set."""
    if is_dividend(name) and DIVIDEND_MONTHS:
        return [(y, m) for y, m in months if m in DIVIDEND_MONTHS]
    return list(months)
...
def build_contracts(products, months):
    ...
    for root, name, key in product_rows(products):
        cs = []
        for y, m in product_months(name, months):
...
def fetch_contract_price(bbg, c):
    """Dividend futures: fill c.px_rows with the contract's own daily PRICE_FIELD over exactly the
    window its open interest was pulled for (nothing to do when the contract has no OI rows)."""
    c.px_rows = []
    if c.status != OK or not c.rows:
        return
    c.px_rows = bbg.history(c.ticker, PRICE_FIELD, c.req_start, c.req_end)
...
def resolve_indices(bbg, results):
    ...
    A dividend future (DIVIDEND_FUTURES) has no index: its series is own_price, carries only the
    FX rate, and Bloomberg is not asked about any index for it.  Its contract currency is
    CONTRACT_CURRENCY[tab], else Bloomberg's CRNCY on the futures (noted, since that field is not
    trusted in general).
    """
    out = {}
    for name, cs in results:
        if is_dividend(name):
            out[name] = IndexSeries(product=name, ticker='', own_price=True, index_source='DIVIDEND_FUTURES',
                                    bbg_fut_currency=product_currency(cs))
            continue
    ...
    wanted = sorted({ix.ticker for ix in out.values() if ix.ticker})
    ref = bbg.ref(wanted, INDEX_REF_FIELDS) if wanted else {}
    for ix in out.values():
        if ix.own_price:
            ...
            elif ix.bbg_fut_currency:
                ix.fut_currency, ix.ccy_source = ix.bbg_fut_currency, 'futures CRNCY'
                ix.note = ("contract currency %s taken from Bloomberg's CRNCY on the futures - list %s in "
                           'CONTRACT_CURRENCY to pin it' % (ix.fut_currency, ix.product))
            else:
                ix.note = ('%s is a dividend future with no CRNCY on its contracts and no CONTRACT_CURRENCY entry, '
                           'so the contract currency is unknown' % ix.product)
                continue
...
def fetch_index(bbg, ix, contracts, today):
    ...
    if ix.own_price:                                # dividend future: no index, just the rate over the same span
        if ix.fx_ticker:
...
def compute_notional(contracts, ix):
    ...
    print on or before that day.  On a dividend future (ix.own_price) the contract's own price
    stands in for the index: OI x FUT_VAL_PT x last own PRICE_FIELD print on or before the day,
    / the last FX print on or before it.  Contracts without a multiplier (or, on a dividend future,
    without a price), or products without an index, keep an empty notional and say why in the note."""
    own = ix is not None and ix.own_price
    have_index = ix is not None and ix.status == OK and (own or ix.rows)
    ix_dates = [d for d, _ in ix.rows] if (have_index and not own) else []
    fx_dates = [d for d, _ in ix.fx_rows] if own else []
    for c in contracts:
        c.notional = []
        if c.status != OK or not c.rows:
            continue
        ...
        if c.multiplier is None:
            ...
            continue
        if own and not c.px_rows:
            c.note = (c.note + '; ' if c.note else '') + 'no USD notional: no %s prints for %s in its window' % (
                PRICE_FIELD, c.ticker)
            continue
        out = []
        if own:
            px_dates = [d for d, _ in c.px_rows]
            for d, oi in c.rows:
                i = bisect.bisect_right(px_dates, d) - 1
                if i < 0:
                    continue
                usd = oi * c.multiplier * c.px_rows[i][1]         # an amount in the contract's currency
                if ix.fx_ticker:
                    k = bisect.bisect_right(fx_dates, d) - 1
                    if k < 0 or not ix.fx_rows[k][1]:
                        continue
                    rate = ix.fx_rows[k][1]
                    usd = usd * rate if ix.fx_mode == 'multiply' else usd / rate
                out.append((d, usd))
        else:
            for d, oi in c.rows:
        ...
        if len(out) < len(c.rows):
            c.note = (c.note + '; ' if c.note else '') + '%d OI day(s) before the first %s print have no notional' % (
                len(c.rows) - len(out), 'own price or FX' if own else 'index')
...
def audit_row(c):
    return [c.product, c.label, c.ticker_1, c.ticker_2, c.ticker or None, c.status,
            ...
            c.note or None, c.underlying or None, c.last_price]
...
def write_contracts_sheet(wb, results):
    ...
    ws.freeze_panes = 'A2'
    for j, w in enumerate([10, 9, 15, 15, 15, 11, 18, 13, 26, 13, 13, 13, 13, 7, 10, 10, 10, 6, 18, 18, 60, 18, 10], start=1):
        ws.column_dimensions[get_column_letter(j)].width = w
    return ws


def index_row(ix):
    return [ix.product, ix.ticker or None, ix.name or None, ix.currency or None, ix.fut_currency or None,
...
def worked_example(contracts, ix):
    ...
    d, usd = c.notional[-1]
    oi = dict(c.rows)[d]
    if ix.own_price:                                   # the contract's own price stands in for the index
        i = bisect.bisect_right([x for x, _ in c.px_rows], d) - 1
        level, what = c.px_rows[i][1], ' (%s %s)' % (c.ticker.split()[0], PRICE_FIELD)
    else:
        i = bisect.bisect_right([x for x, _ in ix.index_rows], d) - 1
        level, what = ix.index_rows[i][1], ''
    if ix.fx_ticker:
    ...
    return 'check: %s on %s: %s x %g x %.2f%s%s = USD %sm' % (
        c.label, d.isoformat(), format(int(oi), ','), c.multiplier, level, what, fx, format(int(round(usd / 1e6)), ','))
...
def print_summary(results, bbg, out, indices=None, used=None):
    ...
    for name, cs in results:
        ...
        if ix is not None:
            ...
            if ix.status == OK:
                fx = (' / %s' % ix.fx_ticker) if ix.fx_ticker else ' (USD-denominated, no FX)'
                mult_text = ' / '.join('%g' % m for m in mult) if mult else '%s ?' % MULTIPLIER_FIELD
                if ix.own_price:
                    print("         notional:   OI x %s x each contract's own %s (dividend future, no index)%s   [contract ccy %s from %s]"
                          % (mult_text, PRICE_FIELD, fx, ix.fut_currency, ix.ccy_source))
                else:
                    print('         notional:   OI x %s x %s (%s, %d rows)%s   [contract ccy %s from %s]' % (
                        mult_text, ix.ticker, ix.index_source, len(ix.rows), fx, ix.fut_currency, ix.ccy_source))
                if ix.note:
...
# ------------------------------------------------------------------- run ---
def stopped_at(x):
    """The security a failed pull was for: the contract's or index's ticker, or the dividend
    product's FX rate (its IndexSeries has no index ticker)."""
    return x.ticker or getattr(x, 'fx_ticker', '') or '?'
...
def run(products=None, data_start=None, out=None, today=None, blpapi_module=None, tickers_only=None,
    ...
    pull_error = None
    try:
        n_ask = sum(len(product_months(name, months)) for _, name, _ in product_rows(products))
        print('Resolving %d contracts for %d products ...' % (n_ask, len(products)))
        try:
        ...
        for name, cs in results:
            ...
            for c in cs:
                if c.status != OK:
                    continue
                if pull_error is not None:
                    c.status, c.note = NOT_PULLED, 'not requested - the pull stopped at %s' % stopped_at(pull_error[0])
                    continue
                field = HIST_FIELD
                try:
                    fetch_open_interest(bbg, c, data_start, today, skip_months,
                                        last_trades.get((name, c.year - 10, c.month)))
                    if indices is not None and is_dividend(name):   # dividend future: its own price too
                        field = PRICE_FIELD
                        fetch_contract_price(bbg, c)
                    print('.', end='', flush=True)
                except Exception as e:                 # keep what we have, say where it stopped
                    pull_error = (c, e, field)
                    c.status, c.note = NOT_PULLED, 'the pull failed here: %s' % e
                    print(' x', flush=True)
            ix = (indices or {}).get(name)
            if ix is not None and ix.status == OK:
                if pull_error is not None:
                    ix.status, ix.note = NOT_PULLED, 'not requested - the pull stopped at %s' % stopped_at(pull_error[0])
                else:
                    try:
                        fetch_index(bbg, ix, cs, today)
                        print(' + %s' % (ix.ticker or ix.fx_ticker or 'USD'), end='', flush=True)
                    except Exception as e:
                        pull_error = (ix, e, INDEX_FIELD)
    ...
    if pull_error is not None:
        ...
        raise StepError('pulling %s for %s (%s %s)' % (field, stopped_at(c), c.product, c.label), e,
...
FAKE_DIV_PRODUCTS = [('DD', 'DIVX')]         # an annual EUR dividend future (DDZ24, DDZ25, DDZ6): notional from its own price
QUARTERLY = (3, 6, 9, 12)
...
def build_universe():
    ...
    u = {}
    for root, _ in FAKE_PRODUCTS + FAKE_KRW_PRODUCTS + FAKE_MID_PRODUCTS + FAKE_DIV_PRODUCTS:
        for y, m in contract_months(2024, 2026):
            if root == 'XP' and m not in QUARTERLY:
                continue                                            # (v)  quarterly-only product
            if root == 'DD' and m != 12:
                continue                                            # a dividend future: December only
...
FAKE_FUT = {'HI': ('HKD', 50.0), 'XP': ('AUD', 25.0), 'QZ': ('USD', 100.0), 'NF': ('USD', 1.0),
            'KM': ('USD', 250000.0), 'MM': ('USD', 1.0), 'DD': ('EUR', 100.0)}
FAKE_UNDERLYING = {'HI': 'HSI Index', 'XP': 'AS51 Index', 'QZ': 'SIMSCI Index', 'KM': 'KOSPI2 Index', 'NF': '', 'MM': '',
                   'DD': 'DIVX Index'}   # root -> (CRNCY as Bloomberg reports it, FUT_VAL_PT); KM says USD like the real terminal
                                         # DD names an index that does not exist here: a dividend future must never ask for it
FAKE_FX = ['USDHKD Curncy', 'USDAUD Curncy', 'USDSGD Curncy', 'USDKRW Curncy', 'USDEUR Curncy']
...
FAKE_SCALE = {'HI': 1.0, 'XP': 0.7, 'QZ': 1.3, 'KM': 1.0, 'NF': 1.0, 'MM': 1.0, 'DD': 0.3}   # per root, in contracts
...
def fake_px(ticker, d):
    """A dividend future's own settlement price: around 100-120 points, drifting up slowly, wobbling
    a little every day, distinct per ticker."""
    s = UNIVERSE[ticker]
    k = TICKER_ID[ticker]
    days = (d - s['listing']).days
    return round(100.0 + 5.0 * ((k * 3) % 5) + days * 0.01 + 0.8 * math.sin(days / 13.0 + k), 2)
...
def fake_rows(ticker, start, end, field=None):
    ...
    PRICE_FIELD on a futures contract answers with its price (fake_px), anything else with fake_oi."""
    s = UNIVERSE.get(ticker)
    if s is None:
        return []
    value = fake_px if (field == PRICE_FIELD and s.get('kind') is None) else fake_oi
    out = []
    if s.get('ghost'):
        g0, g1 = s['ghost']
    ...
    def _hist_events(self, request):
        ...
        for k, sec in enumerate(secs):
            ...
            if sec not in UNIVERSE:
                ...
                continue
            rows = fake_rows(sec, start, end, field)
            half = len(rows) // 2
...
def test_helpers():
    ...
    check('15 real products, distinct legal tab names',
          len({n for _, n in PRODUCTS}) == len(PRODUCTS) == 15
    ...
    check('real roots: the request note + VG / VHO / ES (MTW dropped: no open interest since 2021), tabs named after the underlying index',
          [p[0] for p in PRODUCTS] == ['HI', 'HC', 'HCT', 'KM', 'XP', 'FT', 'TWT', 'FPO', 'HJA', 'QZ', 'VG', 'VHO', 'ES', 'DED', 'ASD']
          and [p[1] for p in PRODUCTS][-5:] == ['SX5E', 'SX5T', 'SPX', 'SX5ED', 'SPXDIVAN'])
...
def test_workbook(results):
    ...
    check('Contracts: header + one row per contract, underlying recorded', ws.max_row == total + 1 and [c.value for c in ws[1]] == AUDIT_COLUMNS
          and ws.cell(2, len(AUDIT_COLUMNS) - 1).value == 'HSI Index' and ws.cell(2, len(AUDIT_COLUMNS)).value is None,
          (ws.max_row, total))
...
def test_dividend():
    """Dividend futures: December only, no index asked for, each contract's own price pulled over
    its OI window, notional = OI x FUT_VAL_PT x own price / FX, the audit and the summary say so."""
    global CONTRACT_CURRENCY, DIVIDEND_FUTURES
    check('CONFIG: SX5ED / SPXDIVAN are dividend futures, listed in PRODUCTS and CONTRACT_CURRENCY, December only, '
          'PX_LAST as the price, DED / ASD in the exchange table',
          DIVIDEND_FUTURES == {'SX5ED', 'SPXDIVAN'} and {'SX5ED', 'SPXDIVAN'} <= {n for _, n in PRODUCTS}
          and CONTRACT_CURRENCY.get('SX5ED') == 'EUR' and CONTRACT_CURRENCY.get('SPXDIVAN') == 'USD'
          and DIVIDEND_MONTHS == (12,) and PRICE_FIELD == 'PX_LAST'
          and EXPECTED_CONTRACT['DED'] == ('EUR', 100) and EXPECTED_CONTRACT['ASD'] == ('USD', 250)
          and is_dividend('SX5ED') and not is_dividend('SX5E'))
    saved = CONTRACT_CURRENCY, DIVIDEND_FUTURES
    CONTRACT_CURRENCY, DIVIDEND_FUTURES = {'DIVX': 'EUR'}, {'DIVX'}
    try:
        _test_dividend()
    finally:
        CONTRACT_CURRENCY, DIVIDEND_FUTURES = saved


def _test_dividend():
    global CONTRACT_CURRENCY
    decs = [(2024, 12), (2025, 12), (2026, 12)]
    check('product_months: a dividend future asks for the Decembers only, an index future for every month',
          product_months('DIVX', TEST_MONTHS) == decs and product_months('HSI', TEST_MONTHS) == TEST_MONTHS
          and [c.label for c in build_contracts(FAKE_DIV_PRODUCTS, TEST_MONTHS)[0][1]] == ['Dec 24', 'Dec 25', 'Dec 26'])
    # the arithmetic, by hand
    d0, d1, d2, d3 = (dt.date(2024, 1, k) for k in (1, 2, 3, 4))
    ix = IndexSeries(product='DIVX', ticker='', own_price=True, status=OK, fut_currency='EUR', fx_ticker='USDEUR Curncy',
                     fx_rows=[(d0, 0.5)])
    c = Contract(product='DIVX', root='DD', year=2024, month=12, label='Dec 24', ticker_1='a', ticker_2='b', ticker='a',
                 status=OK, multiplier=100.0, rows=[(d0, 3.0), (d1, 4.0), (d2, 5.0), (d3, 6.0)],
                 px_rows=[(d1, 10.0), (d3, 20.0)])
    compute_notional([c], ix)
    check('compute_notional (dividend): OI x FUT_VAL_PT x own price (carried forward) / FX, none before the first price',
          c.notional == [(d1, 4 * 100 * 10.0 / 0.5), (d2, 5 * 100 * 10.0 / 0.5), (d3, 6 * 100 * 20.0 / 0.5)]
          and '1 OI day(s) before the first own price or FX print' in c.note, (c.notional, c.note))
    c.note, ix.fx_ticker, ix.fx_rows = '', '', []
    compute_notional([c], ix)
    check('compute_notional (dividend, USD contract): OI x FUT_VAL_PT x own price, no FX',
          c.notional == [(d1, 4000.0), (d2, 5000.0), (d3, 12000.0)], c.notional)
    c.px_rows, c.note = [], ''
    compute_notional([c], ix)
    check('compute_notional (dividend): no price prints -> no notional, the note names the field and the ticker',
          c.notional == [] and 'no USD notional: no PX_LAST prints for a' in c.note, c.note)
    check('a dividend product with no window is NO DATA on the Indices side, not a crash',
          (lambda x: (fetch_index(Bloomberg(blpapi_module=FakeAPI).connect(), x, [], TEST_TODAY), x.status)[1])(
              IndexSeries(product='DIVX', ticker='', own_price=True, status=OK, fx_ticker='USDEUR Curncy')) == NO_DATA)
    # end to end on the fake terminal
    tmp = tempfile.mkdtemp()
    path = os.path.join(tmp, 'div.xlsx')
    FakeSession.instances.clear()
    out, text = quiet(run, products=FAKE_DIV_PRODUCTS, today=TEST_TODAY, blpapi_module=FakeAPI, out=path, **TEST_KW)
    sess = FakeSession.instances[-1]
    refs = [e for e in sess.log if e['op'] == 'ReferenceDataRequest']
    hist = [(e['securities'][0], e['fields'][0], e['settings']['startDate'], e['settings']['endDate'])
            for e in sess.log if e['op'] == 'HistoricalDataRequest']
    check('run(): only the three Decembers are asked about, and never any index (UNDL_SPOT_TICKER ignored)',
          out == path and 'Resolving 3 contracts for 1 products' in text
          and sorted(x for e in refs for x in e['securities']) == ['DDZ24 Index', 'DDZ25 Index', 'DDZ6 Index']
          and not any('DIVX Index' in x for x in hist) and 'NOT FOUND:' not in text, ([e['securities'] for e in refs], text[:400]))
    check('run(): per contract OPEN_INT then PX_LAST over the same window, then the FX rate; nothing else',
          [(t, f) for t, f, _, _ in hist] == [('DDZ24 Index', 'OPEN_INT'), ('DDZ24 Index', 'PX_LAST'),
                                              ('DDZ25 Index', 'OPEN_INT'), ('DDZ25 Index', 'PX_LAST'),
                                              ('DDZ6 Index', 'OPEN_INT'), ('DDZ6 Index', 'PX_LAST'), ('USDEUR Curncy', 'PX_LAST')]
          and all(hist[k][2:] == hist[k + 1][2:] for k in (0, 2, 4)), hist)
    check('run(): the summary states the dividend formula and the currency source, and a worked example with the own price',
          ('%-8s %2d/%d contracts with data - showing USD notional' % ('DIVX', 3, 3)) in text
          and "notional:   OI x 100 x each contract's own PX_LAST (dividend future, no index) / USDEUR Curncy   "
              '[contract ccy EUR from CONTRACT_CURRENCY]' in text
          and 'check:' in text and 'PX_LAST) / ' in text.split('check:')[1].split('\n')[0]
          and 'USDEUR' in text.split('check:')[1].split('\n')[0]
          and 'WARNING' not in text and 'the exchange multiplier' not in text, text[-700:])
    wb = load_workbook(path)
    ws = wb['DIVX']
    check('workbook: a DIVX tab with 3 contracts + Total + Month, the notional chart, then Contracts and Indices',
          wb.sheetnames == ['DIVX', AUDIT_SHEET, INDEX_SHEET] and ws.max_column == 6
          and [ws.cell(2, j).value for j in range(2, 5)] == ['Dec 24', 'Dec 25', 'Dec 26']
          and ws._charts[0].title.tx.rich.p[0].r[0].t == CHART_TITLE['notional'].format(name='DIVX'),
          (wb.sheetnames, ws.max_column))
    # rebuild every cell by hand: OI x 100 x own PX_LAST / USDEUR
    good, why = True, ''
    for j in range(2, 5):
        tk = ws.cell(1, j).value
        for i in range(3, ws.max_row + 1):
            ...
            d = ws.cell(i, 1).value.date()
            fx_d = d
            while not is_session(fx_d, 'fx'):
                fx_d -= DAY
            exp = fake_oi(tk, d) * 100.0 * fake_px(tk, d) / fake_oi('USDEUR Curncy', fx_d)
            if abs(v - exp) > 1e-6 * exp:
                good, why = False, (tk, d, v, exp)
                break
        if not good:
            break
    check('every DIVX cell = OI x 100 x the contract\'s own PX_LAST of that day / USDEUR of that day, by hand', good, why)
    ws = wb[AUDIT_SHEET]
    rows = {(r[0], r[1]): r for r in ws.iter_rows(min_row=2, values_only=True)}
    r = rows[('DIVX', 'Dec 25')]
    dec25 = [c for c in build_contracts(FAKE_DIV_PRODUCTS, TEST_MONTHS)[0][1] if c.label == 'Dec 25'][0]
    check('Contracts tab: 3 rows, multiplier 100, ccy EUR, notional filled, Last price = its last PX_LAST in the window',
          ws.max_row == 4 and r[16] == 100.0 and r[17] == 'EUR' and r[18] and r[19] and r[21] == 'DIVX Index'
          and abs(r[22] - fake_px('DDZ25 Index', last_session_on_or_before(history_cutoff(2025, 12, 4)))) < 1e-9
          and dec25.ticker_2 == 'DDZ25 Index', r)
    ws = wb[INDEX_SHEET]
    r = [x for x in ws.iter_rows(min_row=2, values_only=True)][0]
    check('Indices tab: no index ticker, ccy EUR from CONTRACT_CURRENCY, USDEUR pulled, source DIVIDEND_FUTURES, no index level',
          r[1] is None and r[2] is None and r[4] == 'EUR' and r[5] == 'CONTRACT_CURRENCY' and r[6] == 'EUR'
          and r[7] == 'USDEUR Curncy' and r[8] == OK and r[13] == 0 and r[14] is None and 0.9 < r[15] < 0.95
          and r[16] is None and r[18] == 'DIVIDEND_FUTURES', r)
    # currency not pinned: Bloomberg's CRNCY on the futures is used, with a warning
    CONTRACT_CURRENCY = {}
    bbg = Bloomberg(blpapi_module=FakeAPI).connect()
    res = resolve_contracts(bbg, FAKE_DIV_PRODUCTS, TEST_MONTHS, TEST_TODAY)
    n0 = len(bbg.session.log)
    ix = resolve_indices(bbg, res)['DIVX']
    check('no CONTRACT_CURRENCY entry: the futures\' CRNCY (EUR) is used, noted as a warning, no reference request made',
          ix.status == OK and ix.own_price and ix.fut_currency == 'EUR' and ix.ccy_source == 'futures CRNCY'
          and ix.fx_ticker == 'USDEUR Curncy' and 'list DIVX in CONTRACT_CURRENCY' in ix.note
          and len(bbg.session.log) == n0, (ix, len(bbg.session.log) - n0))
    warn = contract_warnings(res[0][1], ix)
    check('a dividend root outside EXPECTED_CONTRACT is not judged', warn == [], warn)
    # measure 'oi': no price, no FX, no Indices tab
    FakeSession.instances.clear()
    out, text = quiet(run, products=FAKE_DIV_PRODUCTS, today=TEST_TODAY, blpapi_module=FakeAPI, out=path, measure='oi', **TEST_KW)
    sess = FakeSession.instances[-1]
    fields = [e['fields'][0] for e in sess.log if e['op'] == 'HistoricalDataRequest']
    check('run(measure="oi") on a dividend future: OPEN_INT only, no price, no FX, no Indices tab',
          fields == ['OPEN_INT'] * 3 and INDEX_SHEET not in load_workbook(path).sheetnames, fields)
    # a pull that fails on the price: the message names PX_LAST
    class StopsOnPriceSession(FakeSession):
        def sendRequest(self, request):
            if request.operation == 'HistoricalDataRequest' and request.fields[0] == PRICE_FIELD:
                self.log.append(dict(op=request.operation, securities=list(request.securities),
                                     fields=list(request.fields), settings=dict(request.settings)))
                root = FakeElement('RequestFailure', children=[
                    ('reason', fake_complex('reason', [('description', 'Daily capacity reached')]))])
                ...
                return
            FakeSession.sendRequest(self, request)
    CONTRACT_CURRENCY = {'DIVX': 'EUR'}
    out, text = quiet(notebook_main, products=FAKE_DIV_PRODUCTS, today=TEST_TODAY, blpapi_module=api_with(StopsOnPriceSession),
                      out=path, show_charts_=False, **TEST_KW)
    check('a failure on the price pull is reported as PX_LAST for that contract, workbook still written',
          out is None and 'ERROR while pulling PX_LAST for DDZ24 Index (DIVX Dec 24)' in text and os.path.exists(path)
          and 'Traceback' not in text, text[-500:])
    for f in os.listdir(tmp):
        os.remove(os.path.join(tmp, f))
    os.rmdir(tmp)
...
def demo(out=None):
    ...
    saved = CONTRACT_CURRENCY, DIVIDEND_FUTURES
    CONTRACT_CURRENCY, DIVIDEND_FUTURES = dict(CONTRACT_CURRENCY, DIVX='EUR'), DIVIDEND_FUTURES | {'DIVX'}
    try:                                      # the fake dividend future rides along, priced off its own PX_LAST
        run(products=FAKE_PRODUCTS + FAKE_DIV_PRODUCTS, today=TEST_TODAY, blpapi_module=FakeAPI, out=out,
            **dict(TEST_KW, last_expiry_year=2036))
    finally:
        CONTRACT_CURRENCY, DIVIDEND_FUTURES = saved
    print('open it in Excel: 4 product tabs with a chart each (DIVX is the dividend future), plus Contracts and Indices')
