...
# =============================================================================
# v77.1 (29 Jul 2026) — SHORT-WINDOW MODE added as CELL S, IN SAMPLE ONLY.
...
#
# v77.1 fixes a calendar bug that aborted every window with "<target> has
...
# never printed. v77.0 read those rows as missing data. They are not missing —
# they are not the target's trading days. CELL S now aligns to the TARGET's own
# trading calendar and forward-fills the CANDIDATES onto it, and reports how
# many in-window sessions the target did not print (holiday or trading halt).
#
...
# long: the 3-year pull funds universe discovery, the market-cap paths and the
# point-in-time membership snapshots. CELL S slices this object in memory and
# never calls Bloomberg.
...
short = run_short_windows(
    ...
    # basket_size=3,               # leave None: auto-capped by observation count
    # obs_per_leg=4,               # raise to 5-6 to be stricter about legs
    # max_zero_frac=0.34,          # raise if illiquid ASX names are all screened
...
# --- If a window still fails, look at the target's own July prints ----------
# print(data['prices']['GMD AU Equity'].loc['2026-06-25':'2026-07-28'])
# A run of NaN there is a real ASX trading halt, not a bug — the window shrinks
# to the sessions GMD actually traded, and CELL S prints how many it dropped.
...
# relationship. If it has vanished, that is the regime break — a far stronger
# signal than any three-week in-sample R-squared.
