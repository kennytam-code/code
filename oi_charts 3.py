...
underneath needs no edits.  Run the whole file at once:
    ...
    Command line  python oi_charts.py [--tickers-only] [--out FILE] [--years-back N] [--skip-months N] [--no-index]
...
Every product tab also carries its underlying index converted to USD (daily last price / USDxxx
rate) as one more column and as a line on a second, right-hand axis of the chart.
...
# 7. Underlying index in USD.  For every product the index's daily last price is converted to
#    USD and added as one more column on the tab and as a line on a second (right-hand) axis of
#    the chart.  The index ticker defaults to '<tab name> Index' (HSI Index, KOSPI2 Index,
#    AS51 Index ...); INDEX_TICKERS lists the exceptions.  The index's currency comes from its
#    CRNCY field and the rate from FX_TICKER (local per USD, so USD level = index / rate);
#    FX_OVERRIDES swaps in another pair for a currency, e.g. {'AUD': ('AUDUSD Curncy', 'multiply')}.
#    A USD index is used as is.  INDEX_LINE = False switches the whole thing off.
INDEX_LINE = True
INDEX_TICKERS = {
...
INDEX_LABEL = '{index} (USD)'   # row-2 label and legend entry; {index} = ticker without the yellow key
Y2_AXIS_TITLE = 'Index level (USD)'
# ===========================================================================
...
INDEX_COLUMNS = ['Product', 'Index ticker', 'Name', 'Currency', 'FX ticker', 'Status', 'Request start',
                 'Request end', 'First date', 'Last date', 'Rows', 'Last (USD)', 'Note']
INDEX_COLOR = '000000'                   # the index line: black, 1.5 pt, on the right-hand axis
...
@dataclasses.dataclass
class IndexSeries:
    """One product's underlying index, converted to USD."""
    product: str                     # tab name
    ticker: str                      # 'KOSPI2 Index'
    currency: str = ''               # from CRNCY
    name: str = ''
    fx_ticker: str = ''              # 'USDKRW Curncy' ('' for a USD index)
    fx_mode: str = 'divide'          # index / rate, or 'multiply'
    status: str = NOT_FOUND
    req_start: Optional[dt.date] = None
    req_end: Optional[dt.date] = None
    index_rows: List[Tuple[dt.date, float]] = dataclasses.field(default_factory=list)  # local currency
    fx_rows: List[Tuple[dt.date, float]] = dataclasses.field(default_factory=list)
    rows: List[Tuple[dt.date, float]] = dataclasses.field(default_factory=list)        # USD, sorted
    note: str = ''

    @property
    def label(self):
        return INDEX_LABEL.format(index=self.ticker.split()[0])
...
def fx_for(currency):
    """(FX ticker, 'divide' | 'multiply') that turns the index currency into USD; ('' , ...) for USD."""
...
def resolve_indices(bbg, results):
    ...
    out = {name: IndexSeries(product=name, ticker=index_ticker(name)) for name, _ in results}
    ...
    for ix in out.values():
        ...
        if row is None:
            ...
            continue
        ix.currency = str(row.get('CRNCY') or '').strip()
        ix.name = str(row.get('NAME') or '')
        if not ix.currency:
            ix.note = '%s -> no CRNCY, cannot convert to USD' % ix.ticker
            continue
        ix.fx_ticker, ix.fx_mode = fx_for(ix.currency)
        ix.status = OK
    return out
...
def fetch_index_usd(bbg, ix, contracts, today):
    """Pull the index (and its FX rate) over the span of the product's contract windows, convert to USD."""
    if ix.status != OK:
        return
    ...
    if dropped:
        ix.note = '%d index day(s) before the first FX print dropped' % dropped
...
def add_oi_chart(ws, name, found, n_rows, base_year, index=None):
    """One line per contract on the left axis, the USD index on a right-hand axis; blanks are gaps."""
    n_series = len(found)
    ch = LineChart()
    ...
    if index is not None and index.rows:
        col = n_series + 2
        c2 = LineChart()                       # the index: its own (hidden) date axis + right-hand value axis
        c2.display_blanks = 'gap'
        c2.add_data(Reference(ws, min_col=col, max_col=col, min_row=2, max_row=last_row), titles_from_data=True)
        c2.set_categories(Reference(ws, min_col=1, min_row=3, max_row=last_row))
        c2.x_axis = DateAxis(axId=510, crossAx=200)
        c2.x_axis.number_format = 'mmm-yy'
        c2.x_axis.majorTimeUnit = 'months'
        c2.x_axis.delete = True
        c2.y_axis.axId = 200
        c2.y_axis.crossAx = 510
        c2.y_axis.crosses = 'max'
        c2.y_axis.title = Y2_AXIS_TITLE
        ...
        s.graphicalProperties.line.solidFill = INDEX_COLOR
        ch += c2
        n_cols += 1
    ws.add_chart(ch, '%s2' % get_column_letter(n_cols + 3))


def write_product_sheet(wb, name, contracts, base_year, index=None):
    ws = wb.create_sheet(title=safe_sheet_name(name))
    dates, found = align_product(contracts)
    if index is not None and index.rows:
        dates = sorted(set(dates) | {d for d, _ in index.rows})
    ws['A1'] = 'Ticker'
    ws['A2'] = 'Date'
    ...
    ix_col, ix_lookup = None, {}
    if index is not None and index.rows:
        ix_col = len(found) + 2
        ws.cell(row=1, column=ix_col, value=index.ticker)
        ws.cell(row=2, column=ix_col, value=index.label)
        ix_lookup = dict(index.rows)
    for i, d in enumerate(dates, start=3):
        ...
        if ix_col is not None:
            v = ix_lookup.get(d)
            if v is not None:
                ws.cell(row=i, column=ix_col, value=v).number_format = '#,##0.00'
    ws.freeze_panes = 'B3'
    add_oi_chart(ws, name, found, len(dates), base_year, index)
    return ws
...
def index_row(ix):
    return [ix.product, ix.ticker, ix.name or None, ix.currency or None, ix.fx_ticker or None, ix.status,
            ix.req_start, ix.req_end, ix.first_dt, ix.last_dt,
            (len(ix.rows) if ix.status in (OK, NO_DATA) else None), ix.last_value, ix.note or None]
...
def write_indices_sheet(wb, indices):
    ...
    ws.freeze_panes = 'A2'
    for j, w in enumerate([10, 16, 30, 9, 16, 11, 13, 13, 13, 13, 7, 12, 60], start=1):
        ws.column_dimensions[get_column_letter(j)].width = w
    return ws


def write_workbook(path, results, base_year=None, indices=None):
    """indices: {tab name: IndexSeries} from resolve_indices(); None -> no index column / line / tab."""
    if base_year is None:
        base_year = min([c.year for _, cs in results for c in cs] or [START_YEAR])
    wb = Workbook()
    wb.remove(wb.active)
    for name, cs in results:
        write_product_sheet(wb, name, cs, base_year, (indices or {}).get(name))
...
# ------------------------------------------------------- notebook charts ---
def show_charts(results, base_year=None, indices=None):
    ...
    for name, cs in results:
        ...
        if ix is not None and ix.rows:
            ax2 = ax.twinx()
            ax2.plot([d for d, _ in ix.rows], [v for _, v in ix.rows], linewidth=1.4,
                     color='#' + INDEX_COLOR, label=ix.label)
            ax2.set_ylabel(Y2_AXIS_TITLE)
            ax2.yaxis.set_major_formatter(FuncFormatter(lambda v, _pos: format(int(v), ',')))
        ...
        for side in ('top', 'right'):
            ax.spines[side].set_visible(ax2 is not None and side == 'right')
        fig.autofmt_xdate()
        handles, labels = ax.get_legend_handles_labels()
        if ax2 is not None:
            h2, l2 = ax2.get_legend_handles_labels()
            handles, labels = handles + h2, labels + l2
        ax.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, -0.14), ncol=12, fontsize=7,
...
def print_summary(results, bbg, out, indices=None):
    print()
    for name, cs in results:
        ...
        if ix is not None:
            if ix.status == OK:
                print('         index:      %s in USD via %s, %d rows' % (ix.ticker, ix.fx_ticker or 'no FX (USD index)',
                                                                          len(ix.rows)))
            else:
...
def run(products=None, start_year=None, end_year=None, years_back_n=None, out=None,
        ...
        host=BBG_HOST, port=BBG_PORT, skip_months=None, index_line=None):
    ...
    skip_months = SKIP_MONTHS if skip_months is None else skip_months
    index_line = INDEX_LINE if index_line is None else index_line
    ...
    print('OI charts | %d products | contracts %s .. %s | %d years back per contract | '
          'last %d month(s) before expiry dropped | today %s'
          % (len(products), month_label(*months[0]), month_label(*months[-1]), years_back_n,
             skip_months, today.isoformat()))
    try:
    ...
    try:
        ...
        for name, cs in results:
            ...
            if ix is not None and ix.status == OK:
                if pull_error is not None:
                    ...
                    try:
                        fetch_index_usd(bbg, ix, cs, today)
        ...
        bbg.close()
    try:
        write_workbook(out, results, base_year=start_year, indices=indices)
    except Exception as e:
        raise StepError('writing the workbook %s' % out, e)
    print_summary(results, bbg, out, indices)
    if show_charts_:
        try:
            show_charts(results, base_year=start_year, indices=indices)
        except Exception as e:
...
def main(argv=None):
    ...
    p.add_argument('--no-index', action='store_true', help='no underlying-index column / line')
    ...
    try:
        run(start_year=a.start_year, end_year=a.end_year, years_back_n=a.years_back,
            ...
            skip_months=a.skip_months, index_line=not a.no_index)
        return 0
    except Exception as e:
        report_error(e)
        return 1
...
FAKE_INDICES = {'HSI Index': 'HKD', 'AS51 Index': 'AUD', 'SIMSCI Index': 'SGD'}
FAKE_FX = ['USDHKD Curncy', 'USDAUD Curncy', 'USDSGD Curncy']
...
def fake_oi(ticker, d):
    ...
    if s.get('kind') == 'fx':
        return 7.7 + (TICKER_ID[ticker] % 3) * 0.02 + (days % 10) * 0.01
...
def test_helpers():
    ...
    check('CONFIG: 3 years back, last 2 months dropped, index line on', YEARS_BACK == 3 and SKIP_MONTHS == 2 and INDEX_LINE)
    ...
    check('fx_for: USDxxx / divide, USD -> none, FX_OVERRIDES respected',
          fx_for('KRW') == ('USDKRW Curncy', 'divide') and fx_for('USD') == ('', 'divide') and fx_for('') == ('', 'divide'))
    global FX_OVERRIDES
    saved = FX_OVERRIDES
    ...
    ix = IndexSeries(product='KOSPI2', ticker='KOSPI2 Index')
    check('IndexSeries label', ix.label == 'KOSPI2 (USD)' and ix.first_dt is None and ix.last_value is None)
...
def test_indices():
    ...
    ix = indices['HSI']
    check('index resolved: currency, name, FX ticker, OK',
          ix.status == OK and ix.currency == 'HKD' and ix.fx_ticker == 'USDHKD Curncy' and ix.fx_mode == 'divide'
          and ix.name == 'FAKE HSI Index', ix)
    ...
    for name, cs in results:
        fetch_index_usd(bbg, indices[name], cs, TEST_TODAY)
    hist = [e for e in bbg.session.log[n0:] if e['op'] == 'HistoricalDataRequest']
    check('per product: PX_LAST of the index, then of the FX rate (started 10 days earlier)',
          len(hist) == 6 and [e['securities'][0] for e in hist[:2]] == ['HSI Index', 'USDHKD Curncy']
          ...
          [(e['securities'], e['settings']['startDate'], e['settings']['endDate']) for e in hist[:2]])
    cs = results[0][1]
    ...
    fx = dict(ix.fx_rows)
    check('USD rows: one per index day, index / same-day FX where the rate printed that day, sorted',
          ...
          and all(a[0] < b[0] for a, b in zip(ix.rows, ix.rows[1:]))
          and all(is_session(d) for d, _ in ix.rows), (len(ix.rows), len(ix.index_rows), ix.note))
    check('every product index OK with rows', all(x.status == OK and x.rows for x in indices.values()),
          [(x.product, x.status, x.note) for x in indices.values()])
    tmp = tempfile.mkdtemp()
    path = os.path.join(tmp, 'ix.xlsx')
    write_workbook(path, results, indices=indices)
    wb = load_workbook(path)
    check('tabs: products, Contracts, Indices', wb.sheetnames == ['HSI', 'AS51', 'SIMSCI', AUDIT_SHEET, INDEX_SHEET], wb.sheetnames)
    with zipfile.ZipFile(path) as z:
        chart_xml = {n: z.read(n) for n in z.namelist() if n.startswith('xl/charts/chart')}
    for name, cs in results:
        ws, ix = wb[name], indices[name]
        dates, found = align_product(cs)
        dates = sorted(set(dates) | {d for d, _ in ix.rows})
        n, N = len(found), len(dates)
        col = n + 2
        check('%s: index column right after the contracts - ticker in row 1, "(USD)" label in row 2, nothing beyond' % name,
              ws.cell(1, col).value == ix.ticker and ws.cell(2, col).value == ix.label and ws.max_column == col,
              (ws.cell(1, col).value, ws.cell(2, col).value, ws.max_column))
        col_a = [v.date() if isinstance(v, dt.datetime) else v for v in (ws.cell(i, 1).value for i in range(3, N + 3))]
        check('%s: column A = union of contract and index dates' % name, col_a == dates and ws.max_row == N + 2)
        lk = dict(ix.rows)
        close = lambda a, b: (a is None and b is None) or (a is not None and b is not None and abs(a - b) <= 1e-9 * abs(b))
        good = all(close(ws.cell(i, col).value, lk.get(d)) for i, d in enumerate(dates, start=3))
        check('%s: every index cell is that day\'s USD level, blank where the index did not print' % name, good)
        ch = ws._charts[0]
        check('%s: two charts combined, index on value axis 200 crossing at max, own hidden date axis' % name,
              len(ch._charts) == 2 and len(ch.series) == n and len(ch._charts[1].series) == 1
              and ch._charts[1].y_axis.axId == 200 and ch._charts[1].y_axis.crosses == 'max'
              and ch._charts[1].y_axis.crossAx == 510 and ch._charts[1].x_axis.axId == 510
              and ch._charts[1].x_axis.delete is True and ch._charts[1].y_axis.delete is False
              and ch._charts[1].y_axis.title.tx.rich.p[0].r[0].t == Y2_AXIS_TITLE)
        s2 = ch._charts[1].series[0]
        cl = get_column_letter(col)
        check('%s: index series from the index column, black 1.5 pt, no markers' % name,
              s2.tx.strRef.f == "'%s'!%s2" % (name, cl) and s2.val.numRef.f == "'%s'!$%s$3:$%s$%d" % (name, cl, cl, N + 2)
              and s2.graphicalProperties.line.solidFill.srgbClr == INDEX_COLOR
              and s2.graphicalProperties.line.width == 19050 and s2.marker.symbol is None and s2.smooth is False,
              (s2.tx.strRef.f, s2.val.numRef.f))
        check('%s: chart anchored one column right of the index column' % name,
              ch.anchor._from.col == n + 3 and ch.anchor._from.row == 1, ch.anchor._from.col)
    check('chart XML: two date axes, two value axes, secondary crosses max, second title present',
          all(x.count(b'<dateAx>') == 2 and x.count(b'<valAx>') == 2 and b'crosses val="max"' in x
              and Y2_AXIS_TITLE.encode() in x and x.count(b'<lineChart>') == 2 for x in chart_xml.values())
          and len(chart_xml) == 3, {k: (x.count(b'<dateAx>'), x.count(b'<valAx>')) for k, x in chart_xml.items()})
    ws = wb[INDEX_SHEET]
    ...
    check('Indices tab: header + one row per product, HSI row complete',
          ...
          and r[1] == 'HSI Index' and r[3] == 'HKD' and r[4] == 'USDHKD Curncy' and r[5] == OK
          and r[6].date() == ix.req_start and r[7].date() == ix.req_end and r[8].date() == ix.first_dt
          and r[9].date() == ix.last_dt and r[10] == len(ix.rows) and abs(r[11] - ix.last_value) < 1e-6, r)
    # no indices -> the old layout exactly (covered by test_workbook) and no Indices tab
    path2 = os.path.join(tmp, 'noix.xlsx')
    write_workbook(path2, results)
    wb2 = load_workbook(path2)
    check('without indices: no Indices tab, no index column, single chart', wb2.sheetnames[-1] == AUDIT_SHEET
          and wb2['HSI'].max_column == 37 and len(wb2['HSI']._charts[0]._charts) == 1)
    # a NOT FOUND / NO DATA index leaves the tab as it was
    indices2 = dict(indices)
    indices2['HSI'] = IndexSeries(product='HSI', ticker='XX Index')
    path3 = os.path.join(tmp, 'partial.xlsx')
    write_workbook(path3, results, indices=indices2)
    wb3 = load_workbook(path3)
    check('index NOT FOUND for one product: that tab has no index column / line, the Indices tab says so',
          wb3['HSI'].max_column == 37 and len(wb3['HSI']._charts[0]._charts) == 1 and wb3['AS51'].max_column == 14
          and [r for r in wb3[INDEX_SHEET].iter_rows(min_row=2, values_only=True)][0][5] == NOT_FOUND)
    # run() end to end, with and without the index line
    ...
    check('run(): index resolved and pulled per product, summary line, Indices tab in the workbook',
          out == path4 and 'HSI Index in USD via USDHKD Curncy' in text and '+ AS51 Index' in text
          and INDEX_SHEET in wb4.sheetnames and wb4['SIMSCI'].max_column == 36
          and sum(1 for e in sess.log if e['op'] == 'HistoricalDataRequest') == 83 + 6, text[-700:])
    out, text = quiet(run, products=FAKE_PRODUCTS, today=TEST_TODAY, blpapi_module=FakeAPI, out=path4, index_line=False)
    wb5 = load_workbook(path4)
    check('run(index_line=False) / --no-index: no index anywhere', INDEX_SHEET not in wb5.sheetnames
          and wb5['HSI'].max_column == 37 and 'Index in USD' not in text)
    if importlib.util.find_spec('matplotlib') is not None:
        import warnings
        import matplotlib.pyplot as plt
        with warnings.catch_warnings():
            warnings.simplefilter('ignore')
            n_fig, _ = quiet(show_charts, results, indices=indices)
        fig = plt.figure(plt.get_fignums()[0])
        n_axes = len(fig.axes)
        y2 = fig.axes[1].get_ylabel() if n_axes > 1 else ''
        n_leg = len(fig.axes[0].get_legend().get_texts())
        last_label = fig.axes[0].get_legend().get_texts()[-1].get_text()
        plt.close('all')
        check('show_charts: right-hand axis with its title, index line in the shared legend',
              n_fig == 3 and n_axes == 2 and y2 == Y2_AXIS_TITLE and n_leg == 37 and last_label == 'HSI (USD)',
              (n_fig, n_axes, y2, n_leg, last_label))
