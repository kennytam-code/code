...
underneath needs no edits.  Run the whole file at once:
    ...
    Command line  python oi_charts.py [--tickers-only] [--out FILE] [--years-back N] [--skip-months N] [--measure oi|notional]
...
#    OI x FUT_VAL_PT x index level (in index points), which is an amount in the contract's
#    currency (CRNCY of the future); it is turned into USD with FX_TICKER (local per USD, so
#    divide).  FX_OVERRIDES swaps in another pair, e.g. {'AUD': ('AUDUSD Curncy', 'multiply')}.
#    A USD-denominated future needs no rate.
INDEX_TICKERS = {
...
INDEX_COLUMNS = ['Product', 'Index ticker', 'Name', 'Index ccy', 'Future ccy', 'FX ticker', 'Status',
                 'Request start', 'Request end', 'First date', 'Last date', 'Rows', 'Last index', 'Last FX', 'Note']
# chart look: dark grey text, light grey axis lines, no gridlines, no borders
CHART_TEXT, CHART_LINE = '404040', 'BFBFBF'
...
class IndexSeries:
    ...
    name: str = ''
    fut_currency: str = ''           # CRNCY of the futures - the currency of FUT_VAL_PT
...
def resolve_indices(bbg, results):
    ...
    The FX pair is chosen by the product's contract currency (that is the currency of FUT_VAL_PT).
    """
    out = {name: IndexSeries(product=name, ticker=index_ticker(name), fut_currency=product_currency(cs))
    ...
    for ix in out.values():
        ...
        if not ix.fut_currency:
            ix.note = 'no resolved contract of %s reports a CRNCY, so the notional cannot be converted' % ix.product
            continue
...
def add_oi_chart(ws, name, found, n_rows, base_year, measure='oi', title=None):
    ...
    ch.y_axis.majorGridlines = None            # no gridlines
...
def write_product_sheet(wb, name, contracts, base_year, measure='oi'):
    """Row 1 tickers, row 2 labels, then one row per date; the cells are the USD notional (or the
    OI when measure == 'oi' or the notional is unavailable).  Returns (sheet, measure used, note)."""
...
def index_row(ix):
    return [ix.product, ix.ticker, ix.name or None, ix.currency or None, ix.fut_currency or None,
            ...
            (len(ix.rows) if ix.status in (OK, NO_DATA) else None), ix.last_index, ix.last_fx, ix.note or None]
...
def write_indices_sheet(wb, indices):
    ...
    ws.freeze_panes = 'A2'
    for j, w in enumerate([10, 16, 30, 9, 10, 16, 11, 13, 13, 13, 13, 7, 12, 10, 60], start=1):
        ws.column_dimensions[get_column_letter(j)].width = w
    return ws


def write_workbook(path, results, base_year=None, indices=None, measure=None):
    """measure: 'notional' (default MEASURE) or 'oi'.  indices: {tab name: IndexSeries} for the
    Indices audit tab (None -> no tab).  Returns {tab name: (measure used, note)}."""
    ...
    used = {}
    for name, cs in results:
        _, m, note = write_product_sheet(wb, name, cs, base_year, measure)
        used[name] = (m, note)
...
# ------------------------------------------------------- notebook charts ---
def show_charts(results, base_year=None, measure=None):
...
def print_summary(results, bbg, out, indices=None, used=None):
    ...
    for name, cs in results:
        ...
        if ix is not None:
            ...
            if ix.status == OK:
                print('         notional:   OI x %s x %s (%d rows)%s' % (
                    ' / '.join('%g' % m for m in mult) if mult else '%s ?' % MULTIPLIER_FIELD, ix.ticker,
                    len(ix.rows), (' / %s' % ix.fx_ticker) if ix.fx_ticker else ' (%s future, no FX)' % ix.fut_currency))
            else:
...
def run(products=None, start_year=None, end_year=None, years_back_n=None, out=None,
        ...
        host=BBG_HOST, port=BBG_PORT, skip_months=None, measure=None):
    ...
    try:
        ...
        bbg.close()
    try:
        used = write_workbook(out, results, base_year=start_year, indices=indices, measure=measure)
    except Exception as e:
        raise StepError('writing the workbook %s' % out, e)
    print_summary(results, bbg, out, indices, used)
    if show_charts_:
        try:
            show_charts(results, base_year=start_year, measure=measure)
        except Exception as e:
...
def main(argv=None):
    ...
    p.add_argument('--measure', choices=('notional', 'oi'), default=MEASURE,
                   help='what the lines are: USD notional or contracts (default %(default)s)')
    ...
    try:
        run(start_year=a.start_year, end_year=a.end_year, years_back_n=a.years_back,
            ...
            skip_months=a.skip_months, measure=a.measure)
        return 0
    except Exception as e:
        report_error(e)
        return 1
...
FAKE_FUT = {'HI': ('HKD', 50.0), 'XP': ('AUD', 25.0), 'QZ': ('USD', 100.0), 'NF': ('USD', 1.0)}   # root -> (CRNCY, FUT_VAL_PT)
...
def test_helpers():
    ...
    check('CONFIG: 3 years back, last 2 months dropped, USD notional', YEARS_BACK == 3 and SKIP_MONTHS == 2 and MEASURE == 'notional')
...
def test_workbook(results):
    ...
    used = write_workbook(path, results, measure='oi')
    ...
    write_workbook(p2, [('EMPTY', empty)], measure='oi')
...
def test_notional():
    ...
    ix = indices['HSI']
    check('index resolved: index ccy, name, future ccy, FX pair of the FUTURE currency, OK',
          ix.status == OK and ix.currency == 'HKD' and ix.fut_currency == 'HKD' and ix.fx_ticker == 'USDHKD Curncy'
          and ix.fx_mode == 'divide' and ix.name == 'FAKE HSI Index', ix)
    qz = indices['SIMSCI']
    check('USD-denominated future on an SGD index: no FX, index used in points',
          qz.status == OK and qz.currency == 'SGD' and qz.fut_currency == 'USD' and qz.fx_ticker == '', qz)
    ...
    for name, cs in results:
        ...
        check('%s: one column per contract, nothing else, dates = union of notional dates' % name,
              ws.max_column == n + 1 and ws.max_row == N + 2
        ...
        ch = ws._charts[0]
        title = ch.title.tx.rich.p[0].r[0].t
        check('%s: single chart, notional title, USD m axis format, no gridlines' % name,
              len(ch._charts) == 1 and len(ch.series) == n and title == CHART_TITLE['notional'].format(name=name)
              ...
              and ch.anchor._from.col == n + 2, (title, ch.anchor._from.col))
    check('chart XML: one line chart, one date + one value axis, no gridlines, USD m format',
          all(x.count(b'<lineChart>') == 1 and x.count(b'<dateAx>') == 1 and x.count(b'<valAx>') == 1
              and b'majorGridlines' not in x and b'#,##0,,&quot;m&quot;' in x for x in chart_xml.values()) and len(chart_xml) == 3)
    ws = wb[AUDIT_SHEET]
    rows = {(r[0], r[1]): r for r in ws.iter_rows(min_row=2, values_only=True)}
    ...
    check('Indices tab: header + one row per product, HSI row complete',
          ...
          and r[1] == 'HSI Index' and r[3] == 'HKD' and r[4] == 'HKD' and r[5] == 'USDHKD Curncy' and r[6] == OK
          and r[7].date() == ix.req_start and r[8].date() == ix.req_end and r[9].date() == ix.first_dt
          and r[10].date() == ix.last_dt and r[11] == len(ix.rows) and abs(r[12] - ix.last_index) < 1e-6
          and abs(r[13] - ix.last_fx) < 1e-9, r)
    r = rows['SIMSCI']
    check('Indices tab: USD future row has no FX ticker / rate', r[5] is None and r[13] is None and r[4] == 'USD', r)
    ...
    check('index NOT FOUND for one product: OI shown there with the fallback title, notional elsewhere',
          ...
          and t == FALLBACK_TITLE.format(name='HSI') and wb3['HSI'].max_column == 37
    ...
    check('run(): USD notional by default - index pulled per product, summary explains the formula, Indices tab',
          ...
          and 'OI x 100 x SIMSCI Index' in text and '(USD future, no FX)' in text and 'showing USD notional' in text
    ...
    if importlib.util.find_spec('matplotlib') is not None:
        ...
        with warnings.catch_warnings():
            ...
            n_fig, _ = quiet(show_charts, results)
        ...
        plt.close('all')
        check('show_charts: notional title and axis, USD m tick labels, no gridlines, one line per contract',
...
def test_run():
    ...
    if importlib.util.find_spec('matplotlib') is None:
        n, text = quiet(show_charts, results, measure='oi')
