...
INDEX_COLUMNS = ['Product', 'Index ticker', 'Name', 'Index ccy', 'Future ccy', 'FX ticker', 'Status',
                 'Request start', 'Request end', 'First date', 'Last date', 'Rows', 'Last index', 'Last FX',
                 'Last index (USD)', 'Note']
...
def write_indices_sheet(wb, indices):
    ...
    ws.freeze_panes = 'A2'
    for j, w in enumerate([10, 16, 30, 9, 10, 16, 11, 13, 13, 13, 13, 7, 12, 10, 14, 60], start=1):
        ws.column_dimensions[get_column_letter(j)].width = w
    return ws
...
def worked_example(contracts, ix):
    ...
    if ix.fx_ticker:
        ...
        fx = ' (%s future, no FX)' % ix.fut_currency
...
FAKE_FUT = {'HI': ('HKD', 50.0), 'XP': ('AUD', 25.0), 'QZ': ('USD', 100.0), 'NF': ('USD', 1.0),
            'KM': ('KRW', 250000.0)}                                # root -> (CRNCY, FUT_VAL_PT)
...
def test_notional():
    ...
    check('Indices tab: header + one row per product, HSI row complete',
          ...
          and abs(r[13] - ix.last_fx) < 1e-9 and abs(r[14] - ix.last_usd) < 1e-9, r)
    r = rows['SIMSCI']
    check('Indices tab: USD future row has no FX ticker / rate, index used as is',
          r[5] is None and r[13] is None and r[4] == 'USD' and r[14] == r[12], r)
    ...
    ws = wb6['KOSPI2']
    check('KOSPI2: the KRW future asks for KOSPI2 Index and USDKRW Curncy after the contracts',
          [e['securities'][0] for e in hist[-2:]] == ['KOSPI2 Index', 'USDKRW Curncy']
          and 'notional:   OI x 250000 x KOSPI2 Index' in text and '/ USDKRW Curncy' in text, text[-600:])
    ...
    check('KOSPI2 Indices row: KRW / KRW / USDKRW Curncy, last index, last FX, last index in USD = index / FX',
          r[3] == 'KRW' and r[4] == 'KRW' and r[5] == 'USDKRW Curncy' and r[6] == OK and 380 < r[12] < 900
          and 1300 < r[13] < 1400 and abs(r[14] - r[12] / r[13]) < 1e-9, r)
