...
# =============================================================================
# v77.2 (29 Jul 2026) — SHORT-WINDOW MODE added as CELL S, IN SAMPLE ONLY.
# CELL 1 and CELL 2 are byte-identical to v75; find_best_hedge() is unchanged.
# CELL 3 runs GMD AU Equity over trailing 2-month and 3-month windows.
#
# v77.2 adds trailing_windows() and a months= shorthand, so window length is a
# parameter rather than hand-typed dates. CELL S now spans roughly 10 to 70
# sessions; the degrees-of-freedom cap scales the basket automatically, so the
# same call is appropriate at both ends of that range.
#
# v77.1 fixed a calendar bug that aborted every window with "<target> has
# missing prices inside ...". data['prices'] is the UNION of all tickers'
# calendars and the universe always contains US-listed factor/global ETFs, so
# each US session on which the ASX was shut appeared as a row where the target
# never printed. Those rows are not missing data — they are not the target's
# trading days. CELL S aligns to the TARGET's own trading calendar, forward-
# fills the CANDIDATES onto it, and reports how many in-window sessions the
# target did not print (holiday or trading halt).
#
# Every number CELL S prints is fitted and scored on the same rows: it is an
# attribution of a window that has already happened, not a forecast.
# =============================================================================
...
def trailing_windows(months=(2, 3), end=None):
    """Windows running back N CALENDAR months from `end` (default: the last
    business day before today).

        trailing_windows((2, 3))                  → 2m and 3m, ending yesterday
        trailing_windows((1, 2, 3), '2026-07-29') → three windows ending 29-Jul

    Calendar months, not trading days: 2m ≈ 42 sessions, 3m ≈ 65. Returns
    [(label, start, end), ...], the same shape run_short_windows expects."""
    ...
    out = []
    for m in months:
        start = (end - pd.DateOffset(months=m)).normalize()
        out.append((f"{m}m ({start:%d-%b} →)", start, end))
    return out
...
def short_window_hedge(
    ...
    # lead=8: the target's own calendar is a subset of the union index, so
    # several prior union rows may be needed to reach one prior TARGET close
    d = slice_data(data, start, end, lead=8)
    ...
    tgt = PORTFOLIO_COL if is_pf else legs[0]
    target_short = pf_label if is_pf else tgt.split(' ')[0]

    # ── align to the TARGET's OWN trading calendar ───────────────────────────
    # data['prices'] is the UNION of every ticker's calendar. The universe
    # always contains US-listed factor/global ETFs (CONTROL_ETFS['FACTOR'] and
    # ['GLOBAL']), so every US session on which the ASX was shut becomes a row
    # where GMD never printed. Those rows are not missing data — they are not
    # the target's trading days. Forward-filling the target across them would
    # inject fake zero returns into a 20-row sample, and treating them as gaps
    # (the v77.0 bug) aborted the whole window.
    # So: keep only the days the target actually traded, then forward-fill the
    # CANDIDATES onto that calendar. A candidate that did not trade carries its
    # last print, i.e. a zero return, which the max_zero_frac screen below
    # catches if it happens chronically.
    if is_pf:
        legs_px = [c for c in legs if c in prices.columns]
        traded = prices[legs_px].notna().all(axis=1)
    else:
        traded = prices[legs[0]].notna()
    n_shut = int((~traded.loc[traded.index >= start]).sum())
    prices = prices.loc[traded]
    pre = prices.index[prices.index < start]
    if len(pre):                      # keep exactly one prior close, so the
        prices = prices.loc[prices.index >= pre[-1]]   # first in-window return
    else:                                              # is a real return
        say(f"    ⚠ no {target_short} print before {start:%d-%b} in the "
            f"download — the first day of this window has no return and is lost")
    if n_shut:
        say(f"    calendar: {n_shut} row(s) inside the window had no "
            f"{target_short} print (exchange holiday, or a halt) — excluded")
    prices = prices.ffill()
    ...
    if tgt not in rets.columns:
        raise ValueError(f"{target_short} is not in the sliced data")
    rets = rets.loc[rets[tgt].notna()]         # drops the leading NaN row
    if len(rets) < 3:
        raise ValueError(
            f"only {len(rets)} usable {target_short} return(s) in {label} "
            f"({start:%d-%b} → {end:%d-%b}). Check the window dates against "
            f"the data you downloaded: data['prices']['{legs[0]}']"
            f".loc['{start:%Y-%m-%d}':'{end:%Y-%m-%d}']")
    ...
    for c in rets.columns:
        if c in target_keepset or c == tgt:
            continue
        s = rets[c]
        if s.isna().any():          # no print at all early in the window —
            skipped['no history'] = skipped.get('no history', 0) + 1
        elif s.std() < 1e-8:
    ...
    if not ok:
        raise ValueError(
            f"no candidate survived screening in {label} "
            f"(skipped: {skipped}). Try min_mktcap_usd_mm lower, "
            f"restrict_to_target_tz=False, or max_zero_frac higher.")
    rets = rets[[tgt] + ok]
    n_obs = len(rets)
...
# ─────────────────────────────────────────────────────────────────────────────
def run_short_windows(target, data, windows=None, end=None, months=None,
                      verbose=True, **kw):
    ...
    months  : trailing calendar months, e.g. months=(2, 3) → a 2-month and a
              3-month window, both ending on `end`. Shorthand for
              windows=trailing_windows(months, end).
    windows : explicit [(label, start, end), ...]; overrides `months`.
              Default when neither is given: month-to-date / from-the-6th /
              from-the-13th, all ending on `end`.
    end     : common end date (e.g. '2026-07-29'). Default: the last business
              day before today, which avoids a partial or not-yet-loaded bar.
    **kw    : passed to short_window_hedge (min_mktcap_usd_mm, force_include,
              exclude, basket_size, max_basket_size, notional_mm,
              restrict_to_target_tz, ...).

    Returns {'runs', 'stability', 'summary', 'core', 'errors'}."""
    if windows is None:
        windows = (trailing_windows(months, end) if months
                   else default_windows(end))
    L = 82
    runs, errs = [], []
    print("=" * L)
...
# =============================================================================
# CELL 3: RUN — GMD AU Equity, trailing 2m and 3m windows, in sample
# =============================================================================
# --- CELL A: download ONCE (the only cell that hits Bloomberg) ---------------
# UNCHANGED from v75. years=3 stays: it funds universe discovery, the
# market-cap paths and the point-in-time membership snapshots. CELL S slices
# this object in memory and never calls Bloomberg.
data = download_hedge_data('GMD AU Equity', min_mktcap_usd_mm=1000)


# --- CELL B: trailing windows — rerun freely, zero API calls ----------------
# months=(2, 3) → a 2-month (~42 sessions) and a 3-month (~65 sessions) window,
# both ending on `end`. Add 1 for a third, shorter comparison point.
short = run_short_windows(
    'GMD AU Equity', data,
    months=(2, 3),                 # trailing CALENDAR months
    end=None,                      # None → last business day before today.
                                   # Pass end='2026-07-29' to include today's
                                   # ASX close once it has loaded.
    min_mktcap_usd_mm=1000,
    restrict_to_target_tz='auto',
    tz_tolerance_hours=3.0,
    max_basket_size=5,             # 42-65 obs supports more legs than the
                                   # 3-week windows did; 7 is defensible at 3m
    notional_mm=None,              # e.g. 10 → adds a "per $10mm" column
    # winsorize_pct=0.02,          # worth switching on at these lengths; at
    #                              # 20 obs it clipped the only information
...
# --- Other window shapes ----------------------------------------------------
# short = run_short_windows('GMD AU Equity', data, months=(1, 2, 3, 6),
#                           min_mktcap_usd_mm=1000)
#
# short = run_short_windows('GMD AU Equity', data,   # the original July set
#                           end='2026-07-28', min_mktcap_usd_mm=1000)
#
# short = run_short_windows('GMD AU Equity', data, windows=[
#     ('pre-event',  '2026-06-15', '2026-07-05'),
#     ('post-event', '2026-07-06', '2026-07-29')],
#     min_mktcap_usd_mm=1000)

# --- If a window fails, look at the target's own prints ---------------------
# print(data['prices']['GMD AU Equity'].loc['2026-04-20':'2026-07-29'])
# A run of NaN is a real ASX trading halt, not a bug — the window shrinks to
# the sessions GMD actually traded, and CELL S prints how many it dropped.

# --- RECOMMENDED PAIRING ----------------------------------------------------
# At 3 months you are still ~65 rows, well short of the 378 find_best_hedge()
# needs. Run the full v75 pipeline on the same 3-year download, then ask
# whether its basket still appears in section [3] above. If it does, the
# recent tape has not broken the relationship. If it has vanished, that is the
# regime break — a far stronger signal than any in-sample R-squared here.
# result = find_best_hedge(
#     'GMD AU Equity', data=data, min_mktcap_usd_mm=1000,
#     restrict_to_target_tz='auto', tz_tolerance_hours=3.0,
#     gross_penalty=0.02, max_gross=None,
#     winsorize_pct=0.01, robust_cov='ledoit',
#     pit_size=True, pit_members=True, mask_stale_days=7, notional_mm=None)
