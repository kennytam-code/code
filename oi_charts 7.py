...
#     band.  Bands are coloured by contract month (neighbouring expiries get strongly different
#     hues) and lighter for older years.  'lines': one line per contract.
CHART_KIND = 'stacked'
...
STACK_HUE_STEP = 150      # degrees between the hues of consecutive contract months: Jan 0, Feb 150, Mar 300 ...


def stack_color(year, month, last_year):
    """Hex colour (no '#') for one contract's band in the stack: hue by month, chosen so that
    neighbouring expiries contrast strongly (quarterlies sit ~90 degrees apart); lighter the older
    the year (last_year = the newest contract year in the workbook)."""
    hue = ((month - 1) * STACK_HUE_STEP % 360) / 360.0
    back = max(0, min(3, last_year - year))
    r, g, b = colorsys.hls_to_rgb(hue, 0.40 + 0.11 * back, 0.62)
...
def stacked_axes(ax, series, dates, last_year, used):
    ...
    for (c, rows), (mode, k, h, b) in zip(series, labels):
        ...
        if mode == 'last':
            ...
            ax.text(k, b + h / 2.0, c.label, fontsize=6.5, color='#' + CHART_TEXT, ha='center', va='center',
                    bbox=dict(boxstyle='round,pad=0.2', fc='white', ec=colour, lw=0.8, alpha=0.9))
            placed += 1
    ...
    for y_anchor, label, colour in sorted(right):
        ...
        ax.annotate(label, xy=(n - 0.5, y_anchor), xytext=(x_text, y_text), fontsize=6.5, color='#' + CHART_TEXT,
                    ha='left', va='center',
                    bbox=dict(boxstyle='round,pad=0.25', fc='white', ec=colour, lw=0.8),
                    arrowprops=dict(arrowstyle='-', color=colour, lw=0.7, shrinkA=0, shrinkB=0))
        placed += 1
    ...
    ax.annotate('%s %s' % (TOTAL_LABEL, value), xy=(n - 1, last_total), xytext=(x_text, max(y_prev + gap, last_total + gap)),
                ...
                bbox=dict(boxstyle='round,pad=0.25', fc='white', ec='#' + TOTAL_COLOR, lw=0.8),
                arrowprops=dict(arrowstyle='-', color='#' + TOTAL_COLOR, lw=0.7, shrinkA=0, shrinkB=0))
...
def test_stacked():
    ...
    check('stack_color: 12 distinct month colours, older years lighter, capped 3 years back',
          len(set(cols)) == 12 and all(len(c) == 6 for c in cols)
          and sum(int(stack_color(2024, 6, 2026)[i:i + 2], 16) for i in (0, 2, 4))
          > sum(int(stack_color(2026, 6, 2026)[i:i + 2], 16) for i in (0, 2, 4))
          and stack_color(2020, 6, 2026) == stack_color(2023, 6, 2026), cols)
    bbg = Bloomberg(blpapi_module=FakeAPI).connect()
    results = resolve_contracts(bbg, FAKE_PRODUCTS, contract_months(2024, 2026), TEST_TODAY)
    for _, cs in results:
        for c in cs:
