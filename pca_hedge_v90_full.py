# =============================================================================
# PCA HEDGE FINDER — complete source, v90 (Fixes 1-90)
# =============================================================================
# Built 29 Jul 2026. Supersedes v84.
#
# HOW TO USE (notebook): run the engine cell once per session, then run
# `hedge_ui()` and drive everything from the form. You never need to read or
# edit the engine, and you never need to know which cell does what.
#
# v90 — usability, with no change to the model:
#   [Fix 85] _validation_tier extracted from find_best_hedge into a shared
#            function, plus preview_window(): what a sample_window will
#            actually buy you (sessions + validation tier) BEFORE a run, from
#            the engine's own rules rather than a second copy that can drift.
#   [Fix 86] Dollarized risk on the ticket: with notional_mm set, tracking
#            error is also shown as the 1-sigma P&L swing per year and per
#            day, based on the WORST validated window; plus a plain-English
#            gloss ("tracks ~65% of the target's moves, cutting ~41% of its
#            risk").
#   [Fix 87] Colour-coded verdict banner in Jupyter (green / amber / red /
#            grey), with the plain-text verdict as the fallback on any
#            rendering failure.
#   [Fix 88] Download cache: every download_hedge_data() result is written to
#            hedge_data_cache/ and reloadable with load_hedge_data() — zero
#            API calls, survives kernel restarts. list_hedge_data() shows
#            what is on disk.
#   [Fix 89] hedge_ui(): an ipywidgets form for target/book, sample window,
#            sizing, universe and advanced settings, with Download / Load /
#            RUN buttons. A thin layer — it assembles the same arguments a
#            code user passes, so results are identical to the scripted path.
#            Every run is kept in UI_STATE and can be exported as code.
#   [Fix 90] The cache is VALIDATED against the book you are hedging rather
#            than reused blindly: a JSON sidecar records the legs, filters
#            and coverage dates, and check_cache_fit() reports exact /
#            compatible (legs changed but all present — the [Fix 66] case) /
#            stale (prices too old) / mismatch (a name was never downloaded).
#            The form shows this live and refuses to run on a mismatch.
#
# Earlier: v84 fixed a REDUCED-tier crash, a bare error on halted targets and
# a confounded staleness metric (now paired); v79 added sample_window, the
# adaptive validation tiers and the rebuilt report; v75 fixed quick_rehedge
# leg churn, ticker validation, sign-aware ranking and added the nested
# pooled R2, bootstrap CI, rolling OOS curve and trade ticket.
#
# Verification (synthetic data; Bloomberg not required): 65-check feature
# suite, 33-check adversarial suite (tier boundaries 12-600 rows, degenerate
# universes, halted target, uncorrelated target refused, cross-timezone x
# short windows, portfolio in every tier, decay calibration) and a 27-check
# UI/cache suite — all passing, including proof that the extracted tier
# function reproduces v84's tier assignments exactly and that preview_window
# agrees with the tier the engine actually runs. Live Bloomberg request flags
# still need one CELL A run against a terminal.
# =============================================================================
...
def find_best_hedge(
    ...
    if val_mode in ('HOLDOUT', 'IN-SAMPLE'):
        ...
        if notional_mm:                                        # [Fix 86]
            ...
            kv('RISK ($)', f"on ${notional_mm:g}mm: "
                           f"±${_te_s*notional_mm:,.2f}mm/yr 1σ ≈ "
                           f"±${_te_s*notional_mm*1e3/np.sqrt(252):,.0f}k/day"
    ...
    if notional_mm:                                           # [Fix 86]
        ...
        kv('RISK ($)', f"on ${notional_mm:g}mm: ±${_te_use*notional_mm:,.2f}mm"
                       f"/yr 1σ ≈ ±${_te_use*notional_mm*1e3/np.sqrt(252):,.0f}k"
                       f"/day ({_basis})")
    if np.isfinite(wdisp['worst_te']):
    ...
    for _i5, (_, cfg) in enumerate(top5.iterrows(), 1):
        ...
        rows5.append([f"#{_i5}", f"lb {_lb5} · pc {_ps5} · k {_bs5}",
                      f"{cfg['avg_r2_oos']:.3f}",
    ...
    _print_table(['', 'CONFIG', 'CV R²', 'UNSEEN R²', 'VOLRED', 'GROSS', ''],
                 rows5)
    if full_rep:
        for _i5, (_, cfg) in enumerate(top5.iterrows(), 1):
            _lb5, _ps5 = int(cfg['lookback']), cfg['pc_spec']
...
# =============================================================================
# CELL 3: FRONT END — [Fix 89] hedge_ui()
# =============================================================================
# A point-and-click form so the tool can be driven without reading or editing
# any code. It is a THIN layer: every button calls the same
# download_hedge_data / find_best_hedge / quick_rehedge with the same
# arguments a code user would pass, so the model, the validation and the
# report are byte-for-byte what the scripted path produces. Nothing about the
# engine changes; the form only assembles arguments.
#
# It also does four things that remove real friction:
#   · previews the validation tier for the chosen window BEFORE a run, using
#     the engine's own _validation_tier ([Fix 85]) — so a window that can only
#     ever give IN-SAMPLE says so in advance, for free;
#   · reloads a CACHED download ([Fix 88]) instead of hitting Bloomberg;
#   · prints the equivalent Python call, so any run can be reproduced,
#     shared or pasted into a script — the GUI is never a dead end;
#   · keeps every result in UI_STATE, so `UI_STATE['result']['weights']` is
#     available in the next cell.
#
# If ipywidgets is missing the function explains how to install it and prints
# the code template instead of failing.
# =============================================================================

UI_STATE = {'data': None, 'result': None, 'code': ''}
...
def hedge_ui(data=None):
    """[Fix 89] Point-and-click front end. Run this and use the form.

    `data` optionally seeds the form with an already-downloaded dataset;
    otherwise use the Data tab to download or to reload a cached one."""
    try:
        ...
        print("ipywidgets is not installed — the form cannot be shown.\n"
              ...
              "kernel)\n\nUse the scripted path meanwhile:\n"
    ...
    LBL = {'description_width': '130px'}
    FULL = W.Layout(width='430px')
    HALF = W.Layout(width='210px')

    # ── target ──────────────────────────────────────────────────────────────
    w_target = W.Textarea(
        value='AAPL US Equity',
        placeholder="AAPL US Equity\nor a book, one per line:\n"
                    "AAPL US Equity, 50\nNVDA US Equity, 30\n"
                    "MSFT US Equity, -20",
        description='Target / book', style=LBL,
        layout=W.Layout(width='430px', height='92px'))
    w_parsed = W.HTML()
    ...
    w_win = W.Dropdown(
        ...
        value='none', description='Sample window', style=LBL, layout=FULL)
    ...
    w_basket = W.Text(value='3,5,7', description='Basket sizes', style=LBL,
                      layout=FULL)
    w_look = W.Text(value='126,252', description='Lookbacks (d)', style=LBL,
                    layout=FULL)
    ...
    # ── advanced ────────────────────────────────────────────────────────────
    w_tz = W.Dropdown(options=[('Auto', 'auto'), ('Force same timezone', True),
                               ('Allow global', False)],
    ...
    w_cov = W.Dropdown(options=[('Ledoit-Wolf shrinkage', 'ledoit'),
                       ...
                       style=LBL, layout=FULL)
    w_gross = W.FloatText(value=0.02, description='Gross penalty', style=LBL,
                          layout=FULL)
    ...
    w_roll = W.Checkbox(value=True, description='Rolling OOS curve',
                        indent=False)
    ...
    def _status(msg, kind='info'):
        col = {'info': '#22436a', 'ok': '#1b6b3a', 'warn': '#8a5a00',
               'err': '#8c1c1c'}[kind]
        w_status.value = (f"<div style='color:{col};font-family:Menlo,"
                          f"monospace;font-size:12px'>{msg}</div>")
        ...
        if len(cat):
            ...
            for _, r in cat.iterrows():
                ...
                if tgt is not None and r.get('_fp'):
                    try:
                        st = check_cache_fit(tgt,
                                             fingerprint=r['_fp'])['status']
    ...
    def _refresh_parsed(_=None):
        try:
            ...
            if isp:
                ...
                w_parsed.value = (f"<span style='color:#1b6b3a;font-size:12px'>"
                                  f"book {lab}: {body} "
                                  f"(gross-normalized)</span>")
            else:
                w_parsed.value = (f"<span style='color:#1b6b3a;font-size:12px'>"
                                  f"single name: {legs[0]}</span>")
        except Exception as e:
    ...
    def _refresh_fit(_=None):
        """[Fix 90] Say plainly whether the loaded download still fits the
        book in the form, and auto-point at a cached set that does."""
        ...
        if fit['status'] == 'mismatch':
            ...
            extra = ('<br>a cached set that fits: <b>'
                     ...
                     'tab' if hit else
                     '<br>press <b>Download from Bloomberg</b> in the Data tab')
        w_fit.value = (f"<div style='color:{col};font-size:12px;"
                       f"font-family:Menlo,monospace'>{mark} data: "
                       f"{fit['message']}{extra}</div>")

    def _best_cache_for(target):
        """[Fix 90] Newest cached download whose columns cover this book."""
        try:
            cat = list_hedge_data()
        except Exception:
            return None
    ...
    def _refresh_tier(_=None):
        ...
        if UI_STATE['data'] is None:
            w_tier.value = ("<span style='color:#8a5a00;font-size:12px'>"
                            "load or download data to preview the validation "
                            "tier</span>")
            return
        try:
            import io as _io
            ...
            buf = _io.StringIO()
            with _rso(buf):
                pv = preview_window(UI_STATE['data'], _window_arg(),
                                    lookback_options=_ui_ints(w_look.value,
                                                              (126, 252)),
                                    basket_size_options=_ui_ints(
                                        w_basket.value, (3, 5, 7)))
            mode = pv.get('mode')
            if mode is None:
    ...
    def on_load(_):
        with out:
            ...
            try:
                ...
                _status(f"loaded · {UI_STATE['data']['prices'].shape[0]} rows "
        ...
        with out:
            ...
            try:
                ...
                _status(f"downloaded and cached · "
                        ...
                        f"{UI_STATE['data']['prices'].shape[1]} tickers", 'ok')
                _refresh_cache()
                _refresh_tier()
                _refresh_fit()
            except Exception as e:
    ...
    def on_run(_):
        with out:
            clear_output()
            if UI_STATE['data'] is None:
                _status('no data yet — use the Data tab to download or load '
                        'a cached set', 'warn')
                return
            try:
                tgt = _ui_parse_book(w_target.value)
            ...
            kw = _kwargs()
            win = _window_arg()
            UI_STATE['code'] = _ui_code(tgt, win,
                                        {k: v for k, v in kw.items()
                                         if k != 'show_plots'},
                                        UI_STATE.get('loaded_from'))
            _status('running…', 'info')
            try:
                UI_STATE['result'] = find_best_hedge(
                    tgt, data=UI_STATE['data'], sample_window=win, **kw)
                _status("done · results in UI_STATE['result'] "
                        "(e.g. UI_STATE['result']['weights'])", 'ok')
            except Exception as e:
    ...
    for wdg in (w_win, w_from, w_to, w_look, w_basket):
        wdg.observe(_refresh_tier, names='value')
    w_target.observe(_refresh_parsed, names='value')

    # ── layout ──────────────────────────────────────────────────────────────
    tab_trade = W.VBox([w_target, w_parsed, W.HTML('<hr>'), w_win, w_dates,
                        w_tier, W.HTML('<hr>'), w_notional, w_mktcap])
    tab_uni = W.VBox([w_basket, w_look, w_force, w_excl, w_tz, w_stale])
    tab_adv = W.VBox([w_cov, w_gross, w_maxgross, w_report,
                      W.HBox([w_plots, w_roll])])
    tab_data = W.VBox([
        W.HTML("<b>Cached downloads</b> — reload without touching Bloomberg"),
        W.HBox([w_cache, b_load]), W.HTML('<hr>'),
        W.HTML("<b>New download</b> — the only action that hits Bloomberg; "
               "the result is cached automatically"),
        w_years, b_dl])
    tabs = W.Tab(children=[tab_trade, tab_uni, tab_adv, tab_data])
    for i, t in enumerate(['Trade', 'Universe', 'Advanced', 'Data']):
        tabs.set_title(i, t)
    ...
    _refresh_fit()
    display(W.VBox([
        W.HTML("<h3 style='margin:4px 0'>PCA Hedge Finder</h3>"
               "<div style='color:#555;font-size:12px;margin-bottom:6px'>"
               "Set the trade up here and press RUN. Results stay in "
               "<code>UI_STATE['result']</code>.</div>"),
        tabs, w_fit, W.HBox([b_run, b_code]), w_status, out]))
    return UI_STATE
